...
class bi_LSTM_model(nn.Module):
    def __init__(self,vocab_size,n_hidden):
        ...
        self.lstm = nn.LSTM(n_hidden,n_hidden,bidirectional=True,num_layers=2,dropout=0.4)
    ...
    def forward(self, X, t, train=True):
        ...
        hidden_state = Variable(torch.zeros(2*2, len(X), n_hidden)).cuda() # [num_layers(=2) * num_directions(=2), batch_size, n_hidden]
        cell_state = Variable(torch.zeros(2*2, len(X), n_hidden)).cuda() # [num_layers(=2) * num_directions(=2), batch_size, n_hidden]
        # final_hidden_state, final_cell_state : [num_layers(=2) * num_directions(=2), batch_size, n_hidden]
